...
$sql = "SELECT * FROM reviewall WHERE kind=1 and faculty=1";
$stmt = $pdo->query($sql);
$firstfirst = $stmt->fetchAll();

$sql = "SELECT * FROM reviewall WHERE kind=1 and faculty=2";
$stmt = $pdo->query($sql);
$firstsecond = $stmt->fetchAll();

$sql = "SELECT * FROM reviewall WHERE kind=1 and faculty=3";
$stmt = $pdo->query($sql);
