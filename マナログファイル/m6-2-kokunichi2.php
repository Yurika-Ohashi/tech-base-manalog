...
$sql = "SELECT * FROM reviewall WHERE faculty=3 and kind=1";
$stmt = $pdo->query($sql);
$firstfirst = $stmt->fetchAll();

$sql = "SELECT * FROM reviewall WHERE faculty=3 and kind=2";
$stmt = $pdo->query($sql);
$firstsecond = $stmt->fetchAll();

$sql = "SELECT * FROM reviewall WHERE faculty=3 and kind=3";
$stmt = $pdo->query($sql);
