...
<br>
<form action="https://tech-base.net/tb-240384/m6-2-semisearch1.php"  method="post">
...
<?php
    foreach($review as $row){
        if($row["faculty"]=="言語文化学部"){
        $id=$row["id"];
        $sql = "SELECT * FROM reviewall WHERE yourid=$id ";
        $stmt = $pdo->query($sql);
        $genbunreview = $stmt->fetchAll();
        foreach($genbunreview as $row){
            echo $row["teacher"]."ゼミ"."　　"."投稿者ID：".$row["yourid"]."　　"."授業名：「".$row["class"]."」　　"."理由：進路選択に役立った"."　　　"."感想：".$row["review"]."<br>";}}}
